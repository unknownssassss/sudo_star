...
if not lang then
   return '*gяσυρ ιѕ αℓяєα∂у α∂∂є∂*'
else
return 'گًٍرًٍوًٍهًٍ دًٍرًٍ لًٍیًٍسًٍتًٍ گًٍرًٍوًٍهًٍ هًٍاًٍیًٍ مًٍدًٍیًٍرًٍیًٍتًٍیًٍ رًٍبًٍاًٍتًٍ هًٍمًٍ اًٍکًٍنًٍوًٍنًٍ مًٍوًٍجًٍوًٍدًٍ اًٍسًٍتًٍ'
  end
end
...
save_data(_config.moderation.data, data) 
if not lang then
 return "*мυтє ¢σηтα¢т* _нαѕ вєєη єηαвℓє∂_"
else
 return "بًٍیًٍصًٍدًٍاًٍ کًٍرًٍدًٍنًٍ مًٍخًٍاًٍطًٍبًٍ فًٍعًٍاًٍلًٍ شًٍدًٍ"
end
end
end
...
local lang = redis:get(hash)
 if not is_mod(msg) then
if not lang then
return "_уσυ'яє ησт_ *мσ∂єяαтσя*"
else
return "شًٍمًٍاًٍ مًٍدًٍیًٍرًٍ گًٍرًٍوًٍهًٍ نًٍمًٍیًٍبًٍاًٍشًٍیًٍدًٍ"
end
end 
...
if not lang then
return "*Mµ†ê Çðñ†å¢†* _Ì§ ÄlrêåÐ¥ Ðï§åßlêÐ_" 
elseif lang then
return "_بًٍیًٍصًٍدًٍاًٍ کًٍرًٍدًٍنًٍ مًٍخًٍاًٍطًٍبًٍ غًٍیًٍرًٍ فًٍعًٍاًٍلًٍ اًٍسًٍتًٍ_"
end
else 
data[tostring(target)]["mutes"]["mute_contact"] = "no"
 save_data(_config.moderation.data, data) 
if not lang then
return "*мυтє ¢σηтα¢т* _нαѕ вєєη ∂ιѕαвℓє∂_" 
else
return "بًٍیًٍصًٍدًٍاًٍ کًٍرًٍدًٍنًٍ مًٍخًٍاًٍطًٍبًٍ غًٍیًٍرًٍ فًٍعًٍاًٍلًٍ شًٍدًٍ"
end
end
end
---------------Mute Forward-------------------
 ...
 tdcli.getChannelFull(msg.chat_id_, group_info, {chat_id=msg.chat_id_,msg_id=msg.id_})
   ...
   local is_link = msg.content_.text_:match("^([https?://w]*.?telegram.me/joinchat/%S+)$") or msg.content_.text_:match("^([https?://w]*.?t.me/joinchat/%S+)$")
    ...
    if matches[1] == 'link' and is_mod(msg) then
      ...
      if not linkgp then
      if not lang then
        return "ƒιяѕт ѕєт α ℓιηк ƒσя gяσυρ ωιтн υѕιηg_ /ѕєтℓιηк"
     else
